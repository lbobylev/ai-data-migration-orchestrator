...
def eyewear_manufacturer_assignment_operation_detection_node(state: MyState) -> MyState:
...
When dealing with operations regarding the `EyewearManufacturerAssignment` asset type, adhere to these guidelines:
### 1. Deletion
In cases where SKUs are mistakenly assigned, rectify the error by removing the assignment.
""",
            ),
            ("user", user_prompt),
        ]
