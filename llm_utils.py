...
class RetryCall(Protocol[T]):
    def __call__(self, fn: Callable[[], T], *, max_attempts: int = 3) -> T: ...
...
class CallWithSelfHeal(Protocol[R]):
    def __call__(
        self,
    ...
    *,
    ...
    while attempt <= max_transient_attempts:
        try:
            ...
            if is_transient_error(e) and attempt < max_transient_attempts:
            ...
            for r in range(max_repairs):
                logger.warning(
                    f"Invalid JSON/validation failed: {last_err}. Repair attempt {r+1}."
                )
                repair_msgs = [
                ...
                try:
                    ...
                    logger.debug(f"Repaired candidate:\n```json\n{json.dumps(raw, indent=2)}\n```")
                    return schema_model.model_validate(raw)
